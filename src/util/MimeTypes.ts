 ...
 * @returns The preferred file extension (without the dot)
 */
 ...
 */
export function getFiltersForMime(mimeType: string): FileFilter[] {
  ...
  if (extension) {
    return [
      { name: mimeType.split('/')[1].toUpperCase(), extensions: [extension] },
      { name: 'All Files', extensions: ['*'] },
    ];
  }
