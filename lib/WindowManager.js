...
import { Settings, Path } from './config/Constants.js'
...
class WindowManager extends EventEmitter {    
    ...
    updateView(show = false) {
        ...
        if (newView.webContents.isLoading()) {
            newView.webContents.prependOnceListener('dom-ready', () => this.#rawWindow.contentView.addChildView(newView))
        } else {
    ...
    #buildViewFindbar(view) {
        ...
        findbar.propagateVisibilityEvents(false)
    ...
    #setBounds(bounds, view = this.#currentView) {
    ...
    #registerWebContentsEvents(webContents) {
        this.#registerStateChangeEvent(webContents, 'muted')
        this.#registerStateChangeEvent(webContents, 'unmuted')
