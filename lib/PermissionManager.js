...
import Manager from "./AppManager.js"
import Storage from "./Storage.js"
import { Path, Permission } from "./config/Constants.js"
import PreferencesManager from "./PreferencesManager.js"
import ScreenShareModal from "./modal/ScreenShareModal.js"
...
class PermissionManager {    
    ...
    #setupPermissionsHandler() {
        ...
        app.prependListener('session-created', /** @param {Electron.Session} s */ s => {
            ...
            s.setBluetoothPairingHandler(this.#requestBluetoothPairing.bind(this))
            // s.setDevicePermissionHandler(/* DEFAULT */)
        })        
    }
    /**
     * @param {Electron.BluetoothPairingHandlerHandlerDetails} _details 
     * @param {(response: Electron.Response) => void} callback
     */
    #requestBluetoothPairing(_details, callback) {
        callback({ confirmed: false })
    }

    /**
     * @param {Electron.DisplayMediaRequestHandlerHandlerRequest} request 
     * @param {(streams: Electron.Streams) => void} callback
     */
    async #shareMedia(request, callback) {
        const currentWindow = Manager.getInstance().getCurrentPage()?.getView()?.rawView
     ...
     * @returns {Boolean} - true if permission is allowed, false if denied, undefined if unknown
     */
     ...
     * @param {Electron.WebContents} webContents 
     * @param {String} permission 
     * @param {(permissionGranted: boolean) => void} callback 
     * @param {(Electron.PermissionRequest) | (Electron.FilesystemPermissionRequest) | (Electron.MediaAccessPermissionRequest) | (Electron.OpenExternalPermissionRequest)} details 
     */
    ...
    /**
     * Determine if the permission is allowed, denied or unknown.
     * @param {Page} page 
     * @param {String} url 
     * @param {String} permission 
     * @returns {Boolean|undefined} - true if allowed, false if denied, undefined if unknown
     */
    #isAllowed(page, url, permission) {
        const view = page.getView()?.rawView
        if (this.#allowTemporaryPermission(view, permission)) { return true }
    ...
    /**
     * Request permission from the user if not already allowed or denied.
     * @param {Page} page 
     * @param {String} url 
     * @param {String[]} permissions 
     * @returns {Promise<Boolean>} - true if all permissions are allowed, false if any is denied
     */
    ...
    async #askPermissionAndSaveStatus(page, data) {
        const parent = WindowManager.getInstance().getRawWindow(false)
        ...
        if (status !== Permission.Status.ALLOW_ONCE) {
            ...
            PreferencesManager.permissionsUpdated()
    ...
    #allowTemporaryPermission(view, permission) {
        if (!view) { return false }
        if (view._hb_permissions === void 0) { return false }
        return view._hb_permissions.includes(permission)
    }
...
export default (() => {
    ...
    return {
        initialize: () => instance ?? (instance = new PermissionManager()),
