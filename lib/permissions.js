...
class Permissions {
    ...
    constructor() {
        this.#screenShareModal = new ScreenShareModal()
    ...
    #setupPermissionsHandler() {
        ...
        app.prependListener('session-created', /** @param {Electron.Session} s */ s => {
            ...
            s.setDisplayMediaRequestHandler(async (r, c) => {
                ...
                const source = await this.#screenShareModal.request({
                    requesterUrl: r.securityOrigin,
                    shareAudioBtn: r.audioRequested,
                    parent: currentWindow
                })

                if (source === void 0) { return c() }
                c({ video: source, audio: r.audioRequested && source.shareAudio })
    ...
    #checkPermissions(webContents, permission, requestingOrigin, details) {
        ...
        console.debug('Permission check:', permission, 'for', url.origin, 'result:', result)
        return result
    }
    ...
    #denyPermissionsOnSession(session) {
        ...
        session.setPermissionCheckHandler((_, _p) => false)
