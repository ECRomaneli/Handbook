        ...
        switch (type) {
            case Permission.Type.MEDIA_ACCESS:
                for (const mediaType of details.mediaTypes) {
                    if (true !== await this.#enqueue(() => this.#requestPermission(page, origin, this.#formatPermission(permission, mediaType)))) {
                        return callback(false)
                    }
                }
                return callback(true)
            default: return callback(true === await this.#enqueue(() => this.#requestPermission(page, origin, permission)))
        }
    }
