...
import { Settings, OS, Path } from './config/Constants.js'
...
class AppManager {
    ...
    #initialize() {
        ...
        AppManager.#systemTheme = nativeTheme.shouldUseDarkColors ? 'dark' : 'light'
        this.#fromClipboardPage = new Page(void 0, 'Clipboard URL')
        nativeTheme.themeSource = Storage.getSettings(Settings.APP_THEME)
        AppManager.setGoogleApiKey(Storage.getSettings(Settings.GOOGLE_API_KEY))
    }
    ...
    #setupTray() {
        this.#tray = new Tray(AppManager.#getTrayIcon(false))
    ...
    #registerPreferencesListeners() {
        PreferencesManager.onPagesUpdated(() => this.#updateAndRefresh())
        PreferencesManager.onSettingsUpdated((_e, id, value) => this.#updateSettings(id, value))
    }
    ...
    #registerGlobalShortcut() {
        ...
        try {
            const ok = globalShortcut.register(this.#globalShortcut, () => { console.debug('toggle', this.#globalShortcut);this.#setupOrTogglePage() })
            ...
            Dialog.confirm(null, {
                    ...
                    if (confirmed) {
                        PreferencesManager.updateSettings(Settings.GLOBAL_SHORTCUT, '')
                        this.#globalShortcut = ''
                    }
                })
        }
    ...
    #setupCurrentPage() {
        ...
        if (!this.#currentPage.hasView()) {
            ...
            NavManager.getInstance().updateContextMenuProvider(() => this.getViewContextMenu()[1].submenu)
            this.#refreshContextMenu()
            this.#updateTrayIcon()
        }
    ...
    #updateTrayIcon() {
        ...
        this.#tray.setImage(AppManager.#getTrayIcon(win.isVisible(true)))
    }
    ...
    #refreshContextMenu() {
        ...
        windowMenuItems.push({
            ...
            click: () => {
                let url = AppManager.#getClipboardImage() ?? AppManager.#getClipboardUrl()
        ...
        windowMenuItems.push({ label: 'Preferences...', click: () => PreferencesManager.open() })

        menuItems.push(...windowMenuItems)
        menuItems.push({ label: 'Exit', click: () => {
            AppManager.showConfirmationDialog({
                title: 'Exit',
    ...
    #createPageSubmenu(page) {
        ...
        return this.#isCurrentPage(page) ?
            [
                ...
                { label: 'Close', click: () => { win.forceClose() } },
                { type: 'separator' },
    ...
    #registerDynamicContextMenu() {
        ...
        const popUpMenu = () => {
            this.#contextMenu.getMenuItemById('clipboard-url').visible = 
                this.#isCurrentPage(this.#fromClipboardPage) || AppManager.#getClipboardUrl() || AppManager.#hasClipboardImage()
    ...
    #updateSettings(id, value) {
        switch (id) {
            ...
            case Settings.GOOGLE_API_KEY:
                AppManager.setGoogleApiKey(value)
                this.#beforeCloseConfirm(
                    'restart-application', 
                    'Restart app?', 
    ...
    #beforeCloseConfirm(id, title, message, confirmAction, onFinally) {
        ...
        PreferencesManager.getWindow().prependOnceListener('close', async e => {
            e.preventDefault()
            try {
                await AppManager.showConfirmationDialog({
                    title: title, message: message, confirmBtn: 'Yes', cancelBtn: 'No', parent: PreferencesManager.getWindow(),
    ...
    async #setupAutoLaunch() {
        try {
            ...
            if (!isEnabled && autoLaunchEnabled === void 0) {
                autoLaunchEnabled = await Dialog.confirm(PreferencesManager.getWindow(), {
                    title: 'Launch on Startup',
                    message: 'Do you want Handbook to launch automatically on startup?',
                    defaultId: 0
                })
                PreferencesManager.updateSettings(Settings.AUTO_LAUNCH, autoLaunchEnabled)
            }
            ...
            const confirmed = await Dialog.confirm(PreferencesManager.getWindow(), {
            ...
            if (confirmed) {
                PreferencesManager.updateSettings(Settings.AUTO_LAUNCH, false)
                await this.#autoLauncher.disable()
            }
        }
    }
    ...
    static #getTrayIcon(open) {
        ...
        if (OS.IS_DARWIN) { return AppManager.#getTrayIconPath('light', open) }
        ...
        if (theme === 'system') {
            ...
            theme = OS.IS_WIN32 ? nativeTheme.shouldUseDarkColorsForSystemIntegratedUI ? 'dark' : 'light' :
                    ...
                    AppManager.#systemTheme
        } else if (theme === 'preferred') {
            theme = nativeTheme.shouldUseDarkColors ? 'dark' : 'light'
        }
        return AppManager.#getTrayIconPath(theme, open)
    }
...
export default (() => {
    /** @type {AppManager} */ let instance
    return { start: () => instance ?? (instance = new AppManager()), getInstance: () => instance }
})()