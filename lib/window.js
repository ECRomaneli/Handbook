...
class WindowWrapper {
    ...
    #unbindWindow() {
        this.#internalWindow.removeAllListeners()
        this.forceClose()
    ...
    #buildFindbar() {
        this.#findbar = Findbar.from(this.#internalWindow)
    ...
    loadURL(url, options) {
        ...
        this.#internalWindow.loadURL(url, options)
    }
    ...
    loadFile(filePath, options) {
        ...
        this.#internalWindow.loadFile(filePath, options)
    }
    ...
    toggleFindbar() {
        const findbar = Findbar.from(this.#internalWindow)
    ...
    recreateWindow(options) {
        ...
        const oldWindow = this.#internalWindow
    ...
    #mute() {
        this.#internalWindow.webContents.setAudioMuted(true)
        this.bus.emit('muted')
    }

    #unmute() {
        this.#internalWindow.webContents.setAudioMuted(false)
        this.bus.emit('unmuted')
    }
    ...
    isFocused() { return this.#internalWindow.isFocused() }
    focus() { this.#internalWindow.focus() }
    openFindbar() { Findbar.from(this.#internalWindow).open() }
    closeFindbar() { Findbar.from(this.#internalWindow).close() }
    isFindbarFocused() { return Findbar.from(this.#internalWindow).isFocused() }
    show() { this.#internalWindow.show() }
    hide() { this.#internalWindow.hide() }
    getTitle() { return this.#internalWindow.getTitle() }
    getCurrentURL() { return this.#internalWindow.webContents.getURL() }
    getBounds() { return this.#internalWindow.getBounds() }
    setBounds(bounds) { this.#internalWindow.setBounds(bounds) }
    goBack() { this.#internalWindow.webContents.navigationHistory.goBack() }
    goForward() { this.#internalWindow.webContents.navigationHistory.goForward() }
    openDevTools() { this.#internalWindow.webContents.openDevTools() }
    reload() { this.#internalWindow.reload() }
    ...
    emitEvent(eventName, ...args) {
        this.#internalWindow.webContents.send(eventName, ...args)
    }

    unload() {
        if (this.#internalWindow.webContents?.getURL() !== WindowWrapper.#BLANK_URL) {
            this.#internalWindow.loadURL(WindowWrapper.#BLANK_URL)
        }
    }
    ...
    #registerStateChangeEvent(win) {
        ...
        registerStateEvent('muted')
        registerStateEvent('unmuted')
