...
import { Positions, Settings } from './config/Constants.js';
...
class Page {
     ...
     * Create view.
     */
    open(contextMenuProvider) {
        if (this.hasView()) { throw new Error('Unexpected window replacement.') }
        this.#view = new ViewWrapper(this.#createViewOptions(), contextMenuProvider)
        this.#view.on('closed', () => { console.debug('Closing view ' + this.#label); this.#view = void 0 })
    ...
    copyFrom(page) {
        ...
        console.debug(this.#session, page.#session, sessionChanged)
    ...
    getPageBounds() {
        if (!this.#hasBounds) {
            this.#hasBounds = true
            if (Page.#resetType) { return this.getDefaultBounds() }
        }
     ...
     */
    getDefaultBounds(forceResetBounds) {
