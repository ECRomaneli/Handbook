...
class ViewWrapper {
     ...
     */
    constructor (parent, options, contextMenuProvider = (() => [])) {
        this.parent = parent
    ...
    #buildFindbar() {
        this.#findbar = Findbar.from(this.parent, this.rawView)

        this.#findbar.setWindowOptions({ alwaysOnTop: true })

        this.#findbar.setWindowHandler(win => {
            win.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true })
            win.prependListener('focus', () => this.parent.emit('modal-focus'))
            win.prependListener('blur', () => this.parent.emit('modal-blur'))
        })
    }
    ...
    loadURL(url, options) {
        ...
        this.rawView.webContents.loadURL(url, options)
    ...
    toggleFindbar() {
        const findbar = Findbar.from(this.parent, this.rawView.webContents)
    ...
    recreateView(options) {
        ...
        options.show = oldView.isVisible()
        ...
        if (this.loaded?.url) {
            // Keep current URL
            newView.loadURL(oldView.webContents.getURL(), this.loaded.options)
        } else if(this.loaded?.filePath) {
            newView.loadFile(this.loaded.filePath, this.loaded.options)
        }

        this.isMuted() && newView.webContents.setAudioMuted(true)
        this.#unbindView(oldView)
    ...
    isFocused() { return this.webContents.isFocused() }
    focus() { this.webContents.focus() }
    openFindbar() { Findbar.from(this.parent, this.webContents).open() }
    closeFindbar() { Findbar.from(this.parent, this.webContents).close() }
    isFindbarFocused() { return Findbar.from(this.parent, this.webContents).isFocused() }
    getTitle() { return this.webContents.getTitle() }
    getBounds() { return this.rawView.getBounds() }
    setBounds(bounds) { this.rawView.setBounds(bounds) }
    ...
    unload() {
        if (this.webContents?.getURL() !== ViewWrapper.#BLANK_URL) {
            this.rawView.loadURL(ViewWrapper.#BLANK_URL)
        }
    }
    ...
    #handleChildWindows(parent) {
        const isView = (win) => !win.isDestroyed() && this.parent.contentView.children.includes(this.rawView)

        parent.webContents
        .on('did-create-window', (childWindow) => {
            const showHandler = () => isView(childWindow) && childWindow.show()
            const hideHandler = () => isView(childWindow) && childWindow.hide()
            this.parent.on('show', showHandler)
            this.parent.on('hide', hideHandler)

            childWindow.once('closed', () => {
                this.parent.off('show', showHandler)
                this.parent.off('hide', hideHandler)
            })
    ...
    static #fixUserAgent(webContents) {
        webContents.setUserAgent(webContents.getUserAgent().replace(/\s(H|h)andbook[^\s]+/, ''))
    }
