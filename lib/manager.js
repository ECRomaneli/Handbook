...
class Manager {
    ...
    #initialize() {
        ...
        winManager.on('window-moved', this.#saveBounds.bind(this, winManager.rawWindow))
        winManager.on('window-resized', this.#saveBounds.bind(this, winManager.rawWindow))
    ...
    #registerPreferencesListeners() {
        Preferences.onPagesUpdated(this.#updateAndRefresh.bind(this))
    ...
    #registerWindowActionAreaListeners() {
        ...
        ipcMain.on('manager.currentPage.dragStart', () => {
            bounds = WindowManager.getInstance().rawWindow.getBounds()
        ...
        ipcMain.on('manager.currentPage.dragging', () => {
            ...
            WindowManager.getInstance().rawWindow.setBounds(bounds)
        })

        ipcMain.on('manager.currentPage.toggleMaximize', () => WindowManager.getInstance().toggleMaximize())
        ipcMain.on('manager.currentPage.hide', () => WindowManager.getInstance().rawWindow.hide())
    }
    ...
    #setupCurrentPage() {
        ...
        if (!this.#currentPage.hasView()) {
            this.#currentPage.open(winManager.rawWindow, () => this.getViewContextMenu())
            this.#refreshContextMenu()
            this.#updateTrayIcon()
        }
        this.#currentPage.updateViewBounds()
        winManager.updateView(this.#currentPage.getView().rawView)
    }
    ...
    #setupAccelerators() {
        const ifVisible = (viewAction) => () => {
            ...
            const win = WindowManager.getInstance().rawWindow
        ...
        const pageMenu = new MenuItem({ label: 'Page', submenu: [
            { label: 'Find...', click: ifVisible(view => view.openFindbar()), accelerator: 'CommandOrControl+F' },
            { label: 'Dismiss', visible: false, click: ifVisible(view => { view.closeFindbar(); view.webContents.focus() }), accelerator: 'Esc' },
    ...
    #createPageSubmenu(page) {
        ...
        return this.#isCurrentPage(page) ?
            [
                ...
                { label: 'Close', click: () => { page.close(); win.unbindView() } },
                { type: 'separator' },
                { label: 'Find...', click: () => view.openFindbar(), visible: win.isVisible(true) },
                { label: 'Back', click: () => view.goBack() },
                { label: 'Forward', click: () => view.goForward() },
                { type: 'separator' },
                { label: 'Refresh', click: () => win.reload() },
                { label: 'Home', click: () => win.reset() },
                { type: 'separator' },
                { label: 'Reset Window', click: () => { page.recreateWindow(); this.#refreshContextMenu() } },
                { label: 'Reset Bounds', click: () => { page.resetWindowBounds() } },
                { type: 'separator' },
                ...
                { label: 'Permissions', click: () => this.#openPermissions(win) }
            ] : 
            [
