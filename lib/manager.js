...
class Manager {
    ...
    #setupCurrentPage() {
        if (!this.#currentPage.hasWindow()) {
            ...
            this.#currentPage.getWindow().bus.on('state-change', () => {
                this.#refreshContextMenu()
                this.#updateTrayIcon()
            })
            this.#refreshContextMenu()
    ...
    #createPageSubmenu(page) {
        ...
        return this.#isCurrentPage(page) ?
            [
                ...
                { label: 'Back', click: () => win.goBack() },
                { label: 'Forward', click: () => win.goForward() },
                { type: 'separator' },
                ...
                { type: 'separator' },
                { label: 'Copy URL', click: () => clipboard.writeText(win.getCurrentURL()) },
                { label: 'Open in Browser', click: () => { shell.openExternal(win.getCurrentURL()) } },
                { label: 'Create Page from URL', click: () => { this.#createNewPageFromUrl() } },
                { type: 'separator' },
                { label: 'Open DevTools', click: () => win.openDevTools() },
                { label: 'Permissions', click: () => {
                    const url = new URL(win.getCurrentURL())
            ...
            [
                ...
                { type: 'separator' },
                { label: 'Permissions', click: () => {
                    const query = 'url: ' + (new URL(win.getCurrentURL())).origin
                    if (Preferences.isOpen()) {
                        Preferences.queryPermissions(query)
                    } else {
                        Preferences.onceReady(() => Preferences.queryPermissions(query)).open()
     ...
     */
    #recreateAllWindows() {
        this.#getAllActivePages().forEach(p => p.recreateWindow(true))
        this.#refreshContextMenu()
    }
