    ...
    #createPageSubmenu(page) {
        ...
        return this.#isCurrentPage(page) ?
            [
                ...
                { label: 'Reload', click: () => view.reset() },
                { label: 'Recreate', click: () => { page.recreateView(); this.#refreshContextMenu() } },
                { label: 'Reset Bounds', click: () => { page.resetViewBounds() } },
                { type: 'separator' },
            ...
            [
                ...
                { type: 'separator' },
                { label: 'Permissions', click: () => {
                    const query = 'url: ' + (new URL(view.webContents.getURL())).origin
                    if (Preferences.isOpen()) {
                        Preferences.queryPermissions(query)
                    } else {
                        Preferences.onceReady(() => Preferences.queryPermissions(query)).open()
