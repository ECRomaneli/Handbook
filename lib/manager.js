import { screen, Notification, Tray, Menu, dialog, globalShortcut, ipcMain, clipboard, app, nativeTheme, MenuItem } from 'electron';
...
const Manager = (() => {
    ...
    return { start: () => instance ?? (instance = new HandbookManager()), getInstance: () => instance }
})()
...
class HandbookManager {
    ...
    #refreshContextMenu() {
        ...
        menuItems.push({ label: 'Exit', click: () => {
            HandbookManager.showConfirmationDialog({
                title: 'Exit',
    ...
    #updateSettings(id, value) {
        switch (id) {
            ...
            case Settings.ALLOW_FULLSCREEN:
                if (this.#confirmationScheduled || !this.#hasAnyActivePage()) { return }
                this.#confirmationScheduled = true
                Preferences.getWindow().prependOnceListener('close', async e => {
                    e.preventDefault()
                    await HandbookManager.showConfirmationDialog({
                        title: 'Recreate all windows?',
                        message: 'Only new windows will receive the new configuration. Do you want to recreate all windows now?',
                        confirmBtn: 'Yes',
                        cancelBtn: 'No',
                        parent: Preferences.getWindow(),
                        confirmAction: () => this.#recreateAllWindows()
                    })
                    this.#confirmationScheduled = false
                    Preferences.close()
                })
                break
