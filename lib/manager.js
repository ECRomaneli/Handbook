import { Tray, Menu, dialog, globalShortcut, ipcMain, clipboard, app, nativeTheme, MenuItem } from 'electron';
import { Storage } from './storage.js';
import { WindowSettings, OS, Path } from './constants.js';
...
class HandbookManager {
    /** @type {Tray} */
    #tray = new Tray(HandbookManager.#getTrayIcon(false))
    ...
    /** @type {Page} */
    #fromClipboardPage = new Page(void 0, 'Clipboard URL')
    ...
    constructor () {
        nativeTheme.themeSource = Storage.getSettings(WindowSettings.APP_THEME)
        this.#updatePages()
    ...
    #updateSettings(id, value) {
        switch (id) {
            ...
            case WindowSettings.GLOBAL_SHORTCUT:
                this.#registerGlobalShortcut()
                break
            case WindowSettings.APP_THEME:
                nativeTheme.themeSource = value
                this.#updateTrayIcon()
                break
            case WindowSettings.TRAY_ICON_THEME:
                this.#updateTrayIcon()
                break
        }
    }
    ...
    static #getTrayIcon(open) {
        ...
        let theme = Storage.getSettings(WindowSettings.TRAY_ICON_THEME)
        if (theme === 'system') {
            theme = nativeTheme.shouldUseDarkColorsForSystemIntegratedUI ? 'dark' : 'light'
        }
