    ...
    #refreshContextMenu() {
        ...
        this.#pages.filter(p => p.canCreateWindow()).forEach(p => 
            windowMenuItems.push({
                ...
                label: p.getLabelWithStatus(this.#isCurrentPage(p)), 
        ...
        windowMenuItems.push({
            id: 'clipboard-url',
            label: this.#fromClipboardPage.getLabelWithStatus(this.#isCurrentPage(this.#fromClipboardPage)), 
            click: () => {
        ...
        if (activePages.length > 0) {
            ...
            if (this.#currentPage?.hasWindow()) {
                ...
                activePagesMenu.submenu.push({
                    label: this.#currentPage.getLabelWithStatus(true),
            ...
            if (activePages.length > 1 || !this.#isCurrentPage(activePages[0])) {
                ...
                otherActivePages.forEach(p => {
                    activePagesMenu.submenu.push({ label: p.getLabelWithStatus(this.#isCurrentPage(p)), submenu: this.#createPageSubmenu(p) })
                })
