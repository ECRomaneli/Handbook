...
class Page {
    ...
    static #DEFAULT_SESSION = 'default'
    ...
    /** @type {ViewWrapper} Window attached to the page. */
    #view
    ...
    constructor (id, label, url, session, persist) {
        ...
        this.#label = label
        this.#url = url
        this.#session = session !== void 0 && session !== '' ? session : Page.#DEFAULT_SESSION
     ...
     * Create view.
     */
    open(parent, contextMenuProvider) {
        if (this.hasView()) { throw new Error('Unexpected window replacement.') }
        this.#view = new ViewWrapper(parent, this.#createViewOptions(), contextMenuProvider)
        this.#view.bus.on('closed', () => { console.log('Closing view ' + this.#label); this.#view = void 0 })
        this.#view.setExternalId(this.#id)
        this.#view.loadURL(this.#url)
        this.updateViewBounds()
    }

    /**
     * Recreate the internal view.
     * @param {true | void} updateOptions If true, the view options are going to be recreated (may change dynamic options).
     * @throws {Error} If there is no view to recreate.
     */
    recreateView(updateOptions) {
        if (!this.hasView()) { throw new Error('No view to recreate.') }
        this.#view.recreateView(updateOptions ? this.#createViewOptions() : void 0)
    }
    ...
    updateViewBounds() {
        ...
        console.debug('Setting bounds for page ' + this.#label, bounds)
        this.#view.setBounds(bounds)
    }

    /**
     * Reset the window bounds to the default size and position.
     */
    resetViewBounds() {
        this.#view.setBounds(this.#resetBounds(true))
    }
    ...
    /**
     * Return whether the page has a view or not.
     * @returns {boolean} True, if there is a view associated to this page.
     */
    hasView() {
     ...
     */
    canCreateWindow() {
        return this.#label && Page.isValidUrl(this.#url)
    }
    ...
    sendToWindow(eventName, ...args) {
        this.#view.emitEvent(eventName, ...args)
    }
     ...
     */
    #createViewOptions() {
        return { webPreferences: { partition: this.getPartitionName() } }
    }
     ...
     */
    #resetBounds(forceResetBounds) {
        const isShared = Storage.getSettings(Settings.SHARE_BOUNDS)
        
        let size

        if (forceResetBounds || Page.#resetType === 'bounds') {
    ...
    getPartitionName() {
        return Storage.getPartitionName(this.#session)
    }

    createNewPageFromUrl() {
        if (!this.hasView()) { throw new Error('No window to create page from current URL.') }
        ...
        return (new Page(void 0, this.#view.getTitle(), url, this.#session, this.#persist)).toPlainPage()
    }
    ...
    toPlainPage() {
        return {
            ...
            label: this.#label,
            url: this.#url,
            session: this.#session !== Page.#DEFAULT_SESSION ? this.#session : '',
