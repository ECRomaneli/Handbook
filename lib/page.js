...
class Page {
    ...
    /** @type {WindowWrapper} Window attached to the page. */
    #window
    ...
    createWindow() {
        ...
        this.#window = new WindowWrapper(this.#createWindowOptions(), () => Manager.getInstance().getWindowContextMenu())
        this.#window.bus.on('closed', () => this.#window = void 0)
     ...
     * @param {true | void} updateOptions If true, the window options are going to be recreated (may change dynamic options).
     * @throws {Error} If there is no window to recreate.
     */
    recreateWindow(updateOptions) {
        if (!this.hasWindow()) {
            throw new Error('No window to recreate.')
        }
        
        this.#window.recreateWindow(updateOptions ? this.#createWindowOptions() : void 0)
    }
    ...
    suspendWindow() {
        if (this.#persist) {
            this.#window.isMaximized() && this.#window.toggleMaximize()
            this.#window.hide()
        } else {
            this.closeWindow()
        }
     ...
     */
    resetWindowBounds() {
        this.#window.setBounds(this.#resetBounds(true))
    }
    ...
    sendToWindow(eventName, ...args) {
        this.#window.emitEvent(eventName, ...args)
    }
    ...
    createNewPageFromUrl() {
        if (!this.hasWindow()) { throw new Error('No window to create page from current URL.') }
        ...
        return (new Page(void 0, this.#window.getTitle(), url, this.#session, this.#persist)).toPlainPage()
    }
