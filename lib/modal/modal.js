...
class Modal {
    ...
    #propagateModalEventsToParent(parent, disableParentEvents) {
        if (disableParentEvents) {
            this.#window.on('closed',        () => { Modal.#setWindowOverlay(parent, false) })
            this.#window.on('ready-to-show', () => { Modal.#setWindowOverlay(parent, true)  })
        }
     ...
     */
    static #setWindowOverlay(window, blocked) {
        if (!window || window.isDestroyed()) return;

        const overlayId = 'modal-parent-overlay';
