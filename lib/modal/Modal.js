...
class Modal {
    ...
    #propagateModalEventsToParent(parent, disableParentEvents) {
        if (disableParentEvents) {
            ...
            this.#window.on('closed',        () => getWebContentsFromWindow(parent).forEach(w => Modal.#setContentOverlay(id, w, false)))
            this.#window.on('ready-to-show', () => getWebContentsFromWindow(parent).forEach(w => Modal.#setContentOverlay(id, w, true)))
        }
    ...
    #registerParentListeners(parent, lockModalToWindow) {
        ...
        if (this.#IS_DARWIN) {
            ...
            parent.once('closed', () => this.#window && !this.#window.isDestroyed() && this.#window.destroy())
        }
     ...
     * @param {Electron.WebContents} webContents - The web contents to create/remove the overlay on.
     * @param {boolean} blocked - true to create, false to remove
     */
    static #setContentOverlay(id, webContents, blocked) {
 ...
 * @param {Electron.BaseWindow | Electron.BrowserWindow} window 
 * @returns {Electron.WebContents[]} Web contents array.
 */
function getWebContentsFromWindow(window) {
    if (!window || window.isDestroyed()) { return [] }
    if (window.contentView?.children.length > 0) { return window.contentView.children.map(child => child.webContents) }
    if (window.webContents) { return [window.webContents] }
    return []
}
